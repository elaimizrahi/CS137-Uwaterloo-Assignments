...
rectangle intersection(rectangle rects[], int n){
    ...
    for(int i = 0; i < n-1; i++){
        for(int j = 0; j < n; j++){
        rectangle a = checkintersection(rects[i], rects[j], width, height);
        //printf("%d %d %d %d\n", a.bottomLeft.x, a.bottomLeft.y, a.width, a.height);
...
printf("result: %d %d %d %d\n", result.bottomLeft.x, result.bottomLeft.y, result.width, result.height);
